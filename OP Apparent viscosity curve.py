...
import time
import shap
data= pd.read_excel('D:\牡蛎流变代码输入数据\训练集牡蛎剪切速率——表观粘度曲线.xlsx')
data.head()
data.isna().sum()
data.info()
cols=data.columns
new_cols=['Shear rate','Apparent viscosity','Cement varieties','Cement content','CP content','NS particle size','NS content','Water content','Total content']
...
data1=pd.read_excel('D:\牡蛎流变代码输入数据\测试集牡蛎剪切速率——表观粘度曲线.xlsx')
...
xgb = XGBRegressor( booster = 'gbtree', learning_rate = 0.01, max_depth = 8, n_estimators = 330,silent=10,random_state =8,objective = 'reg:squarederror',gamma=0.1,subsample=0.8)
...
plt.show()
with open("D:\牡蛎python代码\output_XGBoost表观粘度.xslx", "w") as f:
   for value in y:
